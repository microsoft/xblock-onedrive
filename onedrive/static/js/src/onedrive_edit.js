...
function OneDriveXBlock(runtime, element) {
  ...
  var error_message = $(element).find('.onedrive-xblock .error-message');
  ...
  function isValidUrl(url) {
    ...
    isValid =  /(https?:\/\/(onedrive\.)?)(live\.com)/i.test(url); /*Pattern for 'OneDrive for consumers'*/
    isValid = (isValid == true )? true : /https?:\/\/((\w|-)+)-my.sharepoint.com\//i.test(url)  /*Pattern for 'OneDrive for business'*/
    if(isValid == false)
    {
      isValid = /<iframe/i.test(url);
    }
    alret(isValid);
    return isValid;
  }
